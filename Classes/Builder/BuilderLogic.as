...
void Pickaxe(CBlob@ this)
{
	...
	if (hitdata.blobID == 0)
	{
		...
		if (t != CMap::tile_empty && t != CMap::tile_ground_back)
		{
			...
			if (isTilePlasteel(type) || (this.hasTag("neutral") && isTileIron(type)))
			{
	...
	else
	{
		...
		if (b !is null)
		{
			...
			if (b.hasTag("plasteel_hardness") || (this.hasTag("neutral") && b.hasTag("iron_hardness")))
			{
				this.getSprite().PlaySound("/metal_stone.ogg");
				sparks(b.getPosition(), attackVel.Angle(), 1.0f);
			}
		}
	}
...
void HandlePickaxeCommand(CBlob@ this, u16 blobID, Vec2f tilepos)
{
	...
	if (blobID == 0)
	{
		...
		if (type != CMap::tile_empty && type != CMap::tile_ground_back)
		{
			...
			{
				const bool canHit = this.hasTag("neutral") ? !isTileIron(type) : true;
				if (!isTilePlasteel(type) && canHit)
				{
					map.server_DestroyTile(tilepos, 1.0f, this);
					MaterialsFromTile(this, type, 1.0f, tilepos);
				}
			}
	...
	else
	{
		...
		if (b !is null)
		{
			...
			const bool canHit = this.hasTag("neutral") ? !b.hasTag("iron_hardness") : true;
			if (!b.hasTag("plasteel_hardness") && canHit)
			{
