...
void onInit(CInventory@ this)
{
	...
	{
		BuildBlock[][] blocks;
		if (blob.getConfig() == "peasant")
			addCommonPeasantBlocks(blocks, blob.getTeamNum());
		else
...
void MakeBlocksMenu(CInventory@ this, const Vec2f &in INVENTORY_CE)
{
	...
	const bool isPeasant = blob.getConfig() == "peasant";
	Vec2f menusize = isPeasant ? MENU_SIZE_PEASANT : MENU_SIZE;
	const Vec2f MENU_CE = Vec2f(0, menusize.y * -GRID_SIZE - GRID_PADDING) + INVENTORY_CE;
	...
	if (menu !is null)
	{
		...
		if (isPeasant) return; //peasants dont have pages
