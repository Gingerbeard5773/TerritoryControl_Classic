...
void onTick(CBlob@ this)
{
	...
	{
		CBlob@[] blobsInRadius;
		getMap().getBlobsInRadius(this.getPosition(), this.getRadius() * 1.5f + 25.0f, @blobsInRadius);
		...
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			...
			if (blob is this || !blob.hasTag("seats") || blob.isAttachedToPoint("PICKUP")) continue;

			if (blob.getTeamNum() >= teamsCount || blob.getTeamNum() == this.getTeamNum() || blob.hasTag("usable by anyone"))
			{
		...
		{
			CBlob@ blob = closestAP.getBlob();
			blob.server_AttachTo(this, closestAP);
			if (isServer() && !blob.isAttached())
			{
