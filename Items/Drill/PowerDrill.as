...
const u8 heat_drop = 240;
const u8 high_damage_window = 100; // at how much heat before max drill deals increased damage
...
const f32 heat_reduction_water = 0.15f;
					...
					if (map.getHitInfosFromArc((this.getPosition() - attackVel), -attackVel.Angle(), 30, distance, this, true, @hitInfos))
					{
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							else // map
							{
								...
								const bool plasteel = isTilePlasteel(tile);
								if (isClient() && (plasteel || map.isTileBedrock(tile)))
								{
								...
								if (isServer())
								{
									for (uint i = 0; i < 2; i++)
									{
										...
										MaterialsFromTile(holder, tile, 0.75f, hi.hitpos);
										
										if (map.isTileGround(tile) || map.isTileStone(tile) || map.isTileThickStone(tile) || plasteel) 
										{
										...
										if (plasteel)
										{
											this.server_Hit(this, this.getPosition(), attackVel, 0.05f, Hitters::drill); // Plasteel almost beats mithril drill
											break;
										}
									}
								}
