...
void set_emote(CBlob@ this, string token, int time)
{
	...
	else if (getGameTime() >= this.get_u32("next_emote_sound"))
	{
		const u16 index = emote.index;
		const string CommandID = client ? "server_emotevoice" : "client_emotevoice";
		CBitStream stream;
		stream.write_u16(index);
		this.SendCommand(this.getCommandID(CommandID), stream);
