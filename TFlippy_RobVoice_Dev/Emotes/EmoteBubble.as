...
void onInit(CBlob@ blob)
{
	...
	blob.addCommandID("server_emotevoice");
	blob.addCommandID("client_emotevoice");
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	else if (cmd == this.getCommandID("server_emotevoice") && isServer())
	{
		const u16 index = params.read_u16();
		CBitStream stream;
		stream.write_u16(index);
		this.SendCommand(this.getCommandID("client_emotevoice"), stream);
	}
	else if (cmd == this.getCommandID("client_emotevoice") && isClient())
	{
		const u16 index = params.read_u16();
		if (index < sounds.length && sounds[index] != "")
		{
			f32 pitch = this.getSexNum() == 0 ? 0.9f : 1.5f;
			if (this.exists("voice pitch")) pitch = this.get_f32("voice pitch");

			this.getSprite().PlaySound(sounds[index], 0.80f, pitch);
		}
	}
}
