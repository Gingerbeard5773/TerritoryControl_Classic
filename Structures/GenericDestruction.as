...
void onHealthChange(CBlob@ this, f32 health_old)
{
	...
	const u8 lastFrame = sprite.animation.frame;
	const u8 newFrame = u8((this.getInitialHealth() - this.getHealth()) / (this.getInitialHealth() / sprite.animation.getFramesCount()));
	...
	if (isClient())
	{
		...
		{		
			sprite.PlaySound("/BuildingExplosion", 0.8f, 0.8f);
